...
export async function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('tipOfTheDay.random', () => showRandomTip(tipManager)),
...
async function showRandomTip(tipManager: TipManager) {
    try {
        const tip = await tipManager.randomTip();
        // TODO: Show tip in WebView
        console.log('Showing random tip:', tip);
    } catch (error) {
        vscode.window.showErrorMessage(`Failed to show random tip: ${error}`);
    }
}
