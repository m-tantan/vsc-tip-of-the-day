...
export class OSUtils {
    ...
    private static async initializeOSSettings(): Promise<void> {
        ...
        // If set to auto, detect and update the setting for first time
        if (currentSetting === 'auto') {
            const detectedOS = this.autoDetectOSType();
            await config.update('operatingSystem', detectedOS, vscode.ConfigurationTarget.Global);
        }
...
/**
 * Interface for defining OS-specific shortcuts
 */
